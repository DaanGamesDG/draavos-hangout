  ...
  async run(client: DiscordClient, message: Message, args: Array<string>) {
    ...
    const user: User = message.guild 
      ? client.utils.filterMember(message, args[0] || message.author.id) 
        ...
        : await client.users.fetch(args[0] || message.author.id).catch(e => null)
      : await client.users.fetch(args[0] || message.author.id).catch(e => null);
    ...
    const robloxAccount = data.primaryAccount ? await (await fetch(robloxApi + "users/" + data.primaryAccount)).json() : null;
    ...
    .addField("• Global User Statistics", [
      ...
      `> 🔨 | **Globally banned**: ${banned ? "🔨" : client.utils.EmojiFinder("redtick").toString()}`,
      `> ⚖ | **Conclusion**: ${(rep.upvotes - rep.downvotes < 0 || banned) ? "untrustable" : "trustable"}`
    ])
    .addField("• General Information", [
      `> 👤 | **User**: ${user.toString()}`,
      `> ${client.utils.EmojiFinder("idcard").toString()} | **User ID**: \`${user.id}\``,
      `> 📆 | **Created at**: \`${moment(user.createdTimestamp).format("MMMM Do YYYY hh:mm:ss")} | ${moment(user.createdTimestamp).fromNow()}\``,
    ...
    const member = message.guild ? client.utils.filterMember(message, user.id) : null;
    if (member) {
      ...
      const roles = r.length < 10 ? r.map(role => role.toString()).join(' ') : r.length > 10 ? client.utils.trimArray(r) : "none";

      embed.setColor(member.displayHexColor || "#48B782");
      embed.addField("• Member Information", [
        `> 📆 | **Joined at**: \`${moment(member.joinedTimestamp).format("MMMM Do YYYY hh:mm:ss")} | ${moment(member.joinedTimestamp).fromNow()}\``,
        `> 📂 | **Roles**: ${roles}`
      ]);
    };
