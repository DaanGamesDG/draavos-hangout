...
export default class banCommand extends BaseCommand {
	constructor() {
		super("unban", {
			...
			channelType: "guild",
			description:
				"unban someone in the server or outside the server permanently.",
	...
	async run(client: DiscordClient, message: Message, args: Array<string>) {
		...
		if (!banned)
			return message.channel.send(
				`> ${redtick} | This user isn't banned in this server.`
			);

		const ban = await tempbanSchema.findOne({
			id: user.id,
		...
		await message.guild.members
			.unban(user, `${message.author.id}|${reason}`)
			.catch((e) => {
				return message.channel.send(
					`> ${client.utils
						.EmojiFinder("warning")
