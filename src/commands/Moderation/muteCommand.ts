...
export default class muteCommand extends BaseCommand {
  constructor() {
    super("mute", {
      ...
      usage: "<user id/mention/tag/username>|<duration ex: 1w/30m/60s>|[reason]",
  ...
  async run(client: DiscordClient, message: Message, args: Array<string>) {
    ...
    if (member.id === message.author.id) return message.channel.send("> ❓ | Why do you want to mute yourself?!");
    if (member.id === client.user.id) return message.channel.send("> 😢 | After all the hard work, you still want to mute me?");
    if (member.id === message.guild.ownerID) return message.channel.send("> 👑 | Why do you want to mute the owner? You can't do that!");
    ...
    if (member) {
      if (member.roles.highest.position >= message.member.roles.highest.position && message.guild.ownerID !== message.author.id) 
        return message.channel.send(`> ${redtick} | You cannot mute this user due to role hierarchy.`);
      if (!member.manageable) 
        return message.channel.send(`> ${redtick} | I cannot mute this user due to role hierarchy.`);

      DMed = true;
      await member.send(`> 🔇 | **Muted - Draavo's Hangout**\n> ⌚ | Duration: \`${ms(duration)}\`\n> 📃 | Reason: **${reason}**\n\n> 🙏 | **Want to appeal?** \n Create a ticket with the topic: \`mute appeal\`.`, { split: true })
      .catch(e => DMed = false);
    };
    ...
    setTimeout(() => {
      member.roles.remove(muteRole, `${message.author.id}|automatic unmute from from made ${ms(duration)} ago by ${message.author.tag}`)
        .catch(e => { return message.channel.send(`> ${client.utils.EmojiFinder("warning").toString()} | Oops, Discord threw an exception: \`${e}\`.`) })
      schema.delete();
    }, duration);
